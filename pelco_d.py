...
def Pan_Right_Tilt_Up(addr, pan_speed, tilt_speed):
    'General Response'
    assert 0 <= pan_speed <= 0x40, pan_speed
    assert 0 <= tilt_speed <= 0x3f, tilt_speed
    return Standard_Command(addr, 0, RIGHT + UP, (pan_speed << 8) + tilt_speed)

def Pan_Right_Tilt_Down(addr, pan_speed, tilt_speed):
    'General Response'
    assert 0 <= pan_speed <= 0x40, pan_speed
    assert 0 <= tilt_speed <= 0x3f, tilt_speed
    return Standard_Command(addr, 0, RIGHT + DOWN, (pan_speed << 8) + tilt_speed)

def Pan_Left_Tilt_Up(addr, pan_speed, tilt_speed):
    'General Response'
    assert 0 <= pan_speed <= 0x40, pan_speed
    assert 0 <= tilt_speed <= 0x3f, tilt_speed
    return Standard_Command(addr, 0, LEFT + UP, (pan_speed << 8) + tilt_speed)

def Pan_Left_Tilt_Down(addr, pan_speed, tilt_speed):
    'General Response'
    assert 0 <= pan_speed <= 0x40, pan_speed
    assert 0 <= tilt_speed <= 0x3f, tilt_speed
    return Standard_Command(addr, 0, LEFT + DOWN, (pan_speed << 8) + tilt_speed)
